...
def ChatRoom(request, id):
    ...
    context = {"msg": msg, "usr": usr, "all_usr": usr, "usr2": usr2, "room_name": m_room,
               "notifications": user_notifications, "all_rooms": all_rooms,
               "msgs_room": msgs_room, "msgs": m_arr, "profiles": profiles, "user_profile": user_profile,
...
def CreateGroup(request):
    ...
    print(g_name)
    group = MessageGroup.objects.filter(group_admin=usr).first()
    if group is None:
        group = MessageGroup.objects.create(group_admin=usr)
